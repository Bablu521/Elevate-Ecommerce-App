...
import 'delete_specific_cart_item_use_case_test.mocks.dart';

@GenerateMocks([CartRepo])
void main() {
  group('test DeleteSpecificCartItemUseCase', () {
    late MockCartRepo mockedCartRepo;
    late DeleteSpecificCartItemUseCase deleteSpecificCartItemUseCase;
    final productId = "fake-id";
    setUp(() {
      mockedCartRepo = MockCartRepo();
      deleteSpecificCartItemUseCase = DeleteSpecificCartItemUseCase(
        mockedCartRepo,
      );
    });
    test(
      'when call DeleteSpecificCartItemUseCase it should return CartResponseEntity from repo with correct parameters',
      () async {
        //Arrange
        ...
        when(
          mockedCartRepo.deleteSpecificCartItem(productId),
        ).thenAnswer((_) async => expectedResult);

        //Act
        final result = await deleteSpecificCartItemUseCase.call(productId);

        //Assert
        verify(mockedCartRepo.deleteSpecificCartItem(productId)).called(1);
    ...
    test(
      'when DeleteSpecificCartItemUseCase failed it should return an error result',
      () async {
        //Arrange
